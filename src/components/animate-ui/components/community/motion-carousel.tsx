...
function MotionCarousel(props: PropType) {
  ...
  return (
    <div className="w-full space-y-4 flex-1 [--slide-height:9rem] sm:[--slide-height:13rem] md:[--slide-height:18rem] [--slide-spacing:1.5rem] [--slide-size:55%]">
        ...
        <div className="flex items-center touch-pan-y touch-pinch-zoom">
          {slides.map((src,index) => {
            ...
            return (
              <motion.div
                key={index}
                className="h-full w-full space-x-2 flex-none flex items-center justify-center "
              >
                <motion.img
                  className="size-full shadow-xl  flex items-center justify-center text-3xl md:text-5xl font-semibold select-none border border-red-100 rounded-xl"
                  initial={false}
                  animate={{
...
function DotButton({ selected = false, label, onClick }: DotButtonProps) {
  return (
    <motion.button
      ...
      layout
      initial={false}
      className="flex shadow-xl  cursor-pointer select-none items-center justify-center rounded-full border-none bg-white text-black text-sm"
      animate={{
